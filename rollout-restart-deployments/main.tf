...
resource "aws_sfn_state_machine" "this" {
  ...
  definition = <<-EOT
  {
    "Comment": "call 'kubectl -n $${namespace} rollout restart deployments $${name}'",
    ...
    "States": {
      "GetClusterInfo": {
        ...
        "Next": "CallPatchDeployments"
      },
      "CallPatchDeployments": {
        "Type": "Task",
        ...
        "Parameters": {
          ...
          "Method": "PATCH",
          "Path.$": "States.Format('/apis/apps/v1/namespaces/{}/deployments/{}', $.namespace, $.name)",
          "RequestBody": {
            "spec": {
              "template": {
                "metadata": {
